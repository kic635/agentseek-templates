...
class ProjectSettings(BaseSettings):
    ...
    relay_enabled: bool = Field(default=True, validation_alias="RELAY_ENABLED")
    relay_atof_enabled: bool = Field(default=True, validation_alias="RELAY_ATOF_ENABLED")
    relay_atof_output_dir: str = Field(default=".nemo-relay/atof", validation_alias="RELAY_ATOF_OUTPUT_DIR")
    relay_atof_filename: str = Field(default="events.jsonl", validation_alias="RELAY_ATOF_FILENAME")
    relay_phoenix_enabled: bool = Field(default=True, validation_alias="RELAY_PHOENIX_ENABLED")
    relay_phoenix_endpoint: str = Field(
        default="http://phoenix:6006/v1/traces", validation_alias="RELAY_PHOENIX_ENDPOINT"
    )
    relay_service_name: str = Field(
        default="{{ cookiecutter.project_slug }}", validation_alias="RELAY_SERVICE_NAME"
    )
    relay_project_name: str = Field(
        default="{{ cookiecutter.project_slug }}", validation_alias="RELAY_PROJECT_NAME"
    )
    relay_deployment_environment: str = Field(default="dev", validation_alias="RELAY_DEPLOYMENT_ENVIRONMENT")
